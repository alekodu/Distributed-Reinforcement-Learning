...
//	Implementation using Non-blocking Communication
...
int main (int argc, char **argv) {
	...
	// Create send and receive requests and status
	MPI_Request send_req_right[4], send_req_left[4], send_req_bottom[4], send_req_top[4];
	MPI_Request recv_req_right[4], recv_req_left[4], recv_req_bottom[4], recv_req_top[4];
	MPI_Request reduce_req;
	MPI_Status send_status_right[4], send_status_left[4], send_status_bottom[4], send_status_top[4];
	MPI_Status recv_status_right[4], recv_status_left[4], recv_status_bottom[4], recv_status_top[4];
	...
	while (!global_converged) {
		...
		while (!(trophy_flag && (trophy_state.x == agent_state.x) && (trophy_state.y == agent_state.y)) &&	\
			...
			if (k_send == comm_steps) {
				...
				if (send_flag) {
					if (my_neigh.right) {
						MPI_Waitall(4, send_req_right, send_status_right);
						MPI_Waitall(4, recv_req_right, recv_status_right);
					}
				
					if (my_neigh.left) {
						MPI_Waitall(4, send_req_left, send_status_left);
						MPI_Waitall(4, recv_req_left, recv_status_left);
					}
				
					if (my_neigh.bottom) {
						MPI_Waitall(4, send_req_bottom, send_status_bottom);
						MPI_Waitall(4, recv_req_bottom, recv_status_bottom);
					}
				
					if (my_neigh.top) {
						MPI_Waitall(4, send_req_top, send_status_top);
						MPI_Waitall(4, recv_req_top, recv_status_top);
					}
					
					MPI_Wait(&reduce_req, &reduce_status);
				...
				if (my_neigh.right) {
					MPI_Isend(&qv_current.north[(my_bnds.top_bnd+1)*subgrid.n-2], 1, col_type, my_crds[1]+1, 0, row_comm, &send_req_right[0]);
					MPI_Isend(&qv_current.south[(my_bnds.top_bnd+1)*subgrid.n-2], 1, col_type, my_crds[1]+1, 2, row_comm, &send_req_right[1]);
					MPI_Isend(&qv_current.west[(my_bnds.top_bnd+1)*subgrid.n-2], 1, col_type, my_crds[1]+1, 4, row_comm, &send_req_right[2]);
					MPI_Isend(&qv_current.east[(my_bnds.top_bnd+1)*subgrid.n-2], 1, col_type, my_crds[1]+1, 6, row_comm, &send_req_right[3]);
					
					MPI_Irecv(&qv_current.north[(my_bnds.top_bnd+1)*subgrid.n-1], 1, col_type, my_crds[1]+1, 1, row_comm, &recv_req_right[0]);
					MPI_Irecv(&qv_current.south[(my_bnds.top_bnd+1)*subgrid.n-1], 1, col_type, my_crds[1]+1, 3, row_comm, &recv_req_right[1]);
					MPI_Irecv(&qv_current.west[(my_bnds.top_bnd+1)*subgrid.n-1], 1, col_type, my_crds[1]+1, 5, row_comm, &recv_req_right[2]);
					MPI_Irecv(&qv_current.east[(my_bnds.top_bnd+1)*subgrid.n-1], 1, col_type, my_crds[1]+1, 7, row_comm, &recv_req_right[3]);
				}
				
				if (my_neigh.left) {
					MPI_Isend(&qv_current.north[my_bnds.top_bnd*subgrid.n+1], 1, col_type, my_crds[1]-1, 1, row_comm, &send_req_left[0]);
					MPI_Isend(&qv_current.south[my_bnds.top_bnd*subgrid.n+1], 1, col_type, my_crds[1]-1, 3, row_comm, &send_req_left[1]);
					MPI_Isend(&qv_current.west[my_bnds.top_bnd*subgrid.n+1], 1, col_type, my_crds[1]-1, 5, row_comm, &send_req_left[2]);
					MPI_Isend(&qv_current.east[my_bnds.top_bnd*subgrid.n+1], 1, col_type, my_crds[1]-1, 7, row_comm, &send_req_left[3]);
					
					MPI_Irecv(&qv_current.north[my_bnds.top_bnd*subgrid.n], 1, col_type, my_crds[1]-1, 0, row_comm, &recv_req_left[0]);
					MPI_Irecv(&qv_current.south[my_bnds.top_bnd*subgrid.n], 1, col_type, my_crds[1]-1, 2, row_comm, &recv_req_left[1]);
					MPI_Irecv(&qv_current.west[my_bnds.top_bnd*subgrid.n], 1, col_type, my_crds[1]-1, 4, row_comm, &recv_req_left[2]);
					MPI_Irecv(&qv_current.east[my_bnds.top_bnd*subgrid.n], 1, col_type, my_crds[1]-1, 6, row_comm, &recv_req_left[3]);
				}
				
				// Column communication

				if (my_neigh.bottom) {
					MPI_Isend(&qv_current.north[subgrid.n*(subgrid.m-2)+my_bnds.left_bnd], 1, row_type, my_crds[0]+1, 8, col_comm, &send_req_bottom[0]);
					MPI_Isend(&qv_current.south[subgrid.n*(subgrid.m-2)+my_bnds.left_bnd], 1, row_type, my_crds[0]+1, 10, col_comm, &send_req_bottom[1]);
					MPI_Isend(&qv_current.west[subgrid.n*(subgrid.m-2)+my_bnds.left_bnd], 1, row_type, my_crds[0]+1, 12, col_comm, &send_req_bottom[2]);
					MPI_Isend(&qv_current.east[subgrid.n*(subgrid.m-2)+my_bnds.left_bnd], 1, row_type, my_crds[0]+1, 14, col_comm, &send_req_bottom[3]);
					
					MPI_Irecv(&qv_current.north[subgrid.n*(subgrid.m-1)+my_bnds.left_bnd], 1, row_type, my_crds[0]+1, 9, col_comm, &recv_req_bottom[0]);
					MPI_Irecv(&qv_current.south[subgrid.n*(subgrid.m-1)+my_bnds.left_bnd], 1, row_type, my_crds[0]+1, 11, col_comm, &recv_req_bottom[1]);
					MPI_Irecv(&qv_current.west[subgrid.n*(subgrid.m-1)+my_bnds.left_bnd], 1, row_type, my_crds[0]+1, 13, col_comm, &recv_req_bottom[2]);
					MPI_Irecv(&qv_current.east[subgrid.n*(subgrid.m-1)+my_bnds.left_bnd], 1, row_type, my_crds[0]+1, 15, col_comm, &recv_req_bottom[3]);
				}
				
				if (my_neigh.top) {
					MPI_Isend(&qv_current.north[subgrid.n+my_bnds.left_bnd], 1, row_type, my_crds[0]-1, 9, col_comm, &send_req_top[0]);
					MPI_Isend(&qv_current.south[subgrid.n+my_bnds.left_bnd], 1, row_type, my_crds[0]-1, 11, col_comm, &send_req_top[1]);
					MPI_Isend(&qv_current.west[subgrid.n+my_bnds.left_bnd], 1, row_type, my_crds[0]-1, 13, col_comm, &send_req_top[2]);
					MPI_Isend(&qv_current.east[subgrid.n+my_bnds.left_bnd], 1, row_type, my_crds[0]-1, 15, col_comm, &send_req_top[3]);
					
					MPI_Irecv(&qv_current.north[my_bnds.left_bnd], 1, row_type, my_crds[0]-1, 8, col_comm, &recv_req_top[0]);
					MPI_Irecv(&qv_current.south[my_bnds.left_bnd], 1, row_type, my_crds[0]-1, 10, col_comm, &recv_req_top[1]);
					MPI_Irecv(&qv_current.west[my_bnds.left_bnd], 1, row_type, my_crds[0]-1, 12, col_comm, &recv_req_top[2]);
					MPI_Irecv(&qv_current.east[my_bnds.left_bnd], 1, row_type, my_crds[0]-1, 14, col_comm, &recv_req_top[3]);
				}
				...
				MPI_Iallreduce(&local_converged, &global_converged, 1, MPI_C_BOOL, MPI_LAND, MPI_COMM_WORLD, &reduce_req);
